...
"""
################################################################################
Calibrating Pi Pulse
References: https://qiskit.org/textbook/ch-quantum-hardware/calibrating-qubits-pulse.html
################################################################################
"""
GHz = 1.0e9
MHz = 1.0e6
...
drive_amp = 0.3
drive_pulse = library.gaussian(duration=drive_samples, amp=drive_amp, sigma=drive_sigma,
...
frequency_sweep_program = assemble(schedule, backend=backend, meas_level=1, meas_return='avg',
                                   shots=num_shots_per_frequency, schedule_los=schedule_frequencies)
...
fit_params, y_fit = fit_function(frequencies_GHz, np.real(sweep_values), lambda x, A, q_freq, B, C: (
        (A / np.pi) * (B / ((x - q_freq) ** 2 + B ** 2)) + C), [-5, 4.968, 0.01, 3])
...
rabi_schedules_job = execute(rabi_schedules, backend, shots=8192, meas_level=1,
...
pi_pulse = library.gaussian(duration=drive_samples, amp=pi_amp, sigma=drive_sigma, name='pi_pulse')
direct_rx_schedule = Schedule(Play(pi_pulse, DriveChannel(0)))
# Adding New Definition Of X pulse
...
counts_list = [marginal_counts(cnot_job_result.get_counts(0), meas_qubits=[0, 1]),
               marginal_counts(cnot_job_result.get_counts(1), meas_qubits=[0, 1])]
plot_histogram([counts_list[0], counts_list[1]], title='CNOT Experiment',
               legend=['Standard Pulse', 'Optimized Pulse'])
...
"""
################################################################################
CR Running Experiment Results
Reference: https://github.com/singular-value/optimizations_via_openpulse
################################################################################
"""
x_schedules = []
...
schedules[10].draw(plot_range=[0, 6500], table=True, label=True,
                   channels=[backend_config.drive(0), backend_config.drive(1), backend_config.control(qubits=[0,1])[0]])
schedules[31].draw(plot_range=[0, 3000], table=True, label=True,
                   channels=[backend_config.drive(0), backend_config.drive(1), backend_config.control(qubits=[0,1])[0]])


zz_job = execute(schedules, backend=backend, shots=8192)
job_monitor(zz_job, interval=2)
# zz_job = backend.retrieve_job('5fba7193460f550019dfaa24')
print(zz_job.job_id())

zz_job_result = zz_job.result()
zz_standard_counts_list = []
for i in range(len(thetas)):
    standard_counts = marginal_counts(zz_job_result.get_counts(i), [0, 1])
    zz_standard_counts_list.append(standard_counts)

zz_optimized_counts_list = []
for i in range(len(thetas), len(thetas) * 2):
    optimized_counts = marginal_counts(zz_job_result.get_counts(i), [0, 1])
    zz_optimized_counts_list.append(optimized_counts)

zz_standard_state_vectors = []
for counts in zz_standard_counts_list:
    zz_standard_state_vectors.append(np.array([
        np.sqrt(counts.get('00', 0) / 8192),
        np.sqrt(counts.get('01', 0) / 8192),
        np.sqrt(counts.get('10', 0) / 8192),
        np.sqrt(counts.get('11', 0) / 8192)]))

zz_optimized_state_vectors = []
for counts in zz_optimized_counts_list:
    zz_optimized_state_vectors.append(np.array([
        np.sqrt(counts.get('00', 0) / 8192),
        np.sqrt(counts.get('01', 0) / 8192),
        np.sqrt(counts.get('10', 0) / 8192),
        np.sqrt(counts.get('11', 0) / 8192)]))
...
zz_standard_fidelities = []
for sv1, sv2 in zip(zz_standard_state_vectors, ideal_state_vectors):
    fidelity = state_fidelity(np.abs(sv1), np.abs(sv2))
    zz_standard_fidelities.append(fidelity)

zz_optimized_fidelities = []
for sv1, sv2 in zip(zz_optimized_state_vectors, ideal_state_vectors):
    fidelity = state_fidelity(np.abs(sv1), np.abs(sv2))
    zz_optimized_fidelities.append(fidelity)

cross_fidelities = []
for sv1, sv2 in zip(zz_standard_state_vectors, zz_optimized_state_vectors):
    fidelity = state_fidelity(np.abs(sv1), np.abs(sv2))
    cross_fidelities.append(fidelity)

plt.plot(thetas * 180 / np.pi, zz_standard_fidelities, 'r')
plt.plot(thetas * 180 / np.pi, zz_optimized_fidelities, 'g')
plt.ylabel('Fidelity')
plt.legend(['standard fidelity', 'optimized fidelity'])

"""
################################################################################
Benchmark: QAOA 2 Bit Experiment Job
################################################################################
"""
schedules = []
h_0_h_1_gate = inst_sched_map.get('u2', qubits=[0], P0=0, P1=np.pi) | \
               inst_sched_map.get('u2', qubits=[1], P0=0, P1=np.pi)
cx_01_rx_1_cx01 = inst_sched_map.get('cx', qubits=[0, 1])
cx_01_rx_1_cx01 |= inst_sched_map.get('u1', qubits=[1], P0=np.pi / 2) << cx_01_rx_1_cx01.duration
cx_01_rx_1_cx01 |= inst_sched_map.get('cx', qubits=[0, 1]) << cx_01_rx_1_cx01.duration
rx_0_schedule_pulse = inst_sched_map.get('u3', qubits=[0], P0=3 * np.pi / 4, P1=3 * np.pi / 2, P2=np.pi / 2) | \
                      inst_sched_map.get('u3', qubits=[1], P0=3 * np.pi / 4, P1=3 * np.pi / 2, P2=np.pi / 2)
schedule = h_0_h_1_gate
schedule |= cx_01_rx_1_cx01 << schedule.duration
schedule |= rx_0_schedule_pulse << schedule.duration
schedule |= measure << schedule.duration
schedules.append(schedule)
...
# qaoa_2_bit_job = backend.retrieve_job('5fba26c4891237001ca4f6ff')
...
"""
################################################################################ 
Benchmark: H2 VQE 2 Bit Experiment Job
################################################################################
"""
schedules = []
schedule = (inst_sched_map.get('u2', qubits=[0], P0=0, P1=1) |
            inst_sched_map.get('u2', qubits=[1], P0=-np.pi / 2, P1=np.pi / 2))
...
schedule |= inst_sched_map.get('cx', qubits=[0, 1]) << schedule.duration
schedule |= (inst_sched_map.get('u2', qubits=[0], P0=np.pi, P1=-np.pi) |
             inst_sched_map.get('u2', qubits=[1], P0=-np.pi / 2, P1=np.pi / 2)) << schedule.duration
schedule |= measure << schedule.duration
schedules.append(schedule)

circ = QuantumCircuit(2)
circ.r(np.pi / 2, np.pi / 2, 0)
circ.rx(np.pi / 2, 1)
...
decomposed_circuit = circ.decompose(backend=backend, optimize_using_open_pulse=True)
transpiled_circuit = transpile(decomposed_circuit, backend=backend, optimization_level=1)
...
# h2_vqe_2_bit_job = backend.retrieve_job('5fba69e3447a61001a38165b')
