            ...
            for instruction, qargs, cargs in decomposed_circuit.data:
                ...
                if instruction.name.startswith('directrx'):
                    if not instruction_schedule_map.has(instruction.name, qubits=[qargs[0].index]):
                        ...
                        direct_rx_schedule = Schedule(
                            Play(Waveform(directrx_waveform_array), DriveChannel(qargs[0].index)),
                            name=instruction.name)
                        instruction_schedule_map.add(instruction.name, qubits=[qargs[0].index],
                                                     schedule=direct_rx_schedule)
                    ...
                    if not instruction_schedule_map.has(instruction.name, qubits=[qargs[0].index, qargs[1].index]):
                        ...
                        cx_instructions = instruction_schedule_map.get('cx', qubits=[qargs[0].index,
                                                                                     qargs[1].index]).instructions
                        ...
                        positive_schedule = Schedule(
                            Play(Waveform(crgate_drive_waveform_array_scaled),
                                 crgate_drive_instruction_pulse.channels[0]) |
                            Play(Waveform(crgate_control_waveform_array_scaled),
                                 crgate_control_instruction_pulse.channels[0]),
                            name=instruction.name)
                        inverted_schedule = Schedule(
                            Play(Waveform(-1 * crgate_drive_waveform_array_scaled),
                                 crgate_drive_instruction_pulse.channels[0]) |
                            Play(Waveform(-1 * crgate_control_waveform_array_scaled),
                                 crgate_control_instruction_pulse.channels[0]),
                            name=instruction.name)
                        ...
                        instruction_schedule_map.add(instruction.name, qubits=[qargs[0].index, qargs[1].index],
                                                     schedule=schedule)
